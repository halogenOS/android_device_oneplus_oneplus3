...
HELPER="$XOS_ROOT"/vendor/xos/build/tools/extract_utils.sh
if [ ! -f "$HELPER" ]; then
...
setup_vendor "$DEVICE" "$VENDOR" "$XOS_ROOT"
...
CAMERA_HAL="$XOS_ROOT"/vendor/"$VENDOR"/"$DEVICE"/proprietary/vendor/lib/hw/camera.msm8996.so
